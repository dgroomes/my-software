// 'my-launcher' launches a program based on the properties of a manifest file ('my-launcher.json').
package main
...
// Manifest represents the structure of the manifest file: 'my-launcher.json'
...
func main() {
	...
	if err != nil {
		if os.IsNotExist(err) {
			fatal(fmt.Sprintf("[my-launcher error] Manifest file not found. A 'my-launcher.json' file must be present in the same directory as 'my-launcher'"))
		} else {
			fatal(fmt.Sprintf("[my-launcher error] Unexpected error while reading the manifest file: %v", err))
		}
	}
	...
	if err != nil {
		fatal(fmt.Sprintf("[my-launcher error] JSON parsing error when reading the manifest file: %v", err))
	}

	if manifest.ProgramType != "java" {
		fatal(fmt.Sprintf("[my-launcher error] Only Java programs are supported but 'program_type' was set to '%s'", manifest.ProgramType))
	}
	if manifest.Entrypoint == "" {
		fatal(fmt.Sprintf("[my-launcher error] 'entrypoint' is required in the manifest"))
	}
	if manifest.JavaConfiguration.JavaVersion != 11 && manifest.JavaConfiguration.JavaVersion != 17 && manifest.JavaConfiguration.JavaVersion != 21 {
		fatal(fmt.Sprintf("[my-launcher error] 'java_version' must be one of [11, 17, 21]"))
	}
	...
	if !exists {
		fatal(fmt.Sprintf("[my-launcher error] The manifest requires Java %d, but this version of Java could not be located on the system. The environment variable '%s' must be set", manifest.JavaConfiguration.JavaVersion, envVarName))
	}

	if _, err := os.Stat(javaHome); os.IsNotExist(err) {
		fatal(fmt.Sprintf("[my-launcher error] The environment variable '%s' points to non-existent directory '%s'", envVarName, javaHome))
	}

	javaExecutable := filepath.Join(javaHome, "bin", "java")
	if _, err := os.Stat(javaExecutable); os.IsNotExist(err) {
		fatal(fmt.Sprintf("[my-launcher error] 'java' executable not found at '%s'", javaExecutable))
	}

	javaCmd := buildJavaCommand(&manifest)

	if manifest.DryRun {
		fmt.Println("[my-launcher dry run]")
	...
	if err != nil {
		fatal(fmt.Sprintf("[my-launcher error] Something went wrong while trying to 'exec' the program: %v", err))
	}
}
